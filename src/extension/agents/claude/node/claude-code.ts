...
/**
 * Model information for Claude Code models
 */
...
export const claudeCodeModels = {
	...
	"claude-opus-4-6": {
		...
		reasoningEffort: "high",
		description: "Claude Opus 4.6 - Most capable with adaptive thinking",
 ...
 * - claude-opus-4-5-20251101, claude-opus-4-1-20250805, claude-opus-4-20250514 → claude-opus-4-6
...
export type ClaudeCodeReasoningLevel = keyof typeof claudeCodeReasoningConfig | 'disable' | 'max'

/**
 * Returns true if the given model ID (normalized or raw) is Claude Opus 4.6.
 */
export function isOpus46Model(modelId: string): boolean {
	return normalizeClaudeCodeModelId(modelId) === 'claude-opus-4-6';
}

/**
 * Returns the max output tokens for a given model.
 * Falls back to the model's maxTokens if maxOutputTokens is not defined.
 */
export function getModelMaxOutputTokens(modelId: string): number {
	const normalized = normalizeClaudeCodeModelId(modelId);
	const model: ModelInfo = claudeCodeModels[normalized];
	return model.maxOutputTokens ?? model.maxTokens;
}
