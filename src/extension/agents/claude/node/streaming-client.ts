...
import type { ClaudeCodeRateLimitInfo } from './claude-code';

// Version used for user agent - should match extension version
const CLAUDE_CODE_CLIENT_VERSION = '1.0.0';
...
export const CLAUDE_CODE_API_CONFIG = {
	...
	userAgent: `VSCode-Claude-Code/${CLAUDE_CODE_CLIENT_VERSION}`,
} as const;
...
export async function* createStreamingMessage(options: StreamMessageOptions): AsyncGenerator<StreamChunk> {
	...
	console.log(`[claude-code-streaming] Making request to ${CLAUDE_CODE_API_CONFIG.endpoint}`);
	console.log(`[claude-code-streaming] Model: ${model}`);
	...
	console.log(`[claude-code-streaming] Response status: ${response.status} ${response.statusText}`);
